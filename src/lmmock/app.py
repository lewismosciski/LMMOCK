...
def create_app(storage_dir: Path | None = None) -> FastAPI:
    ...
    async def models(request: Request) -> dict[str, Any]:
        model = store.get_settings().get("model", "mock-model")
        if request.headers.get("anthropic-version"):
            return {"data": [{"type": "model", "id": model, "display_name": "LMMock model", "created_at": "2025-01-01T00:00:00Z"}], "has_more": False, "first_id": model, "last_id": model}
        return {"object": "list", "data": [{"id": model, "object": "model", "created": int(time.time()), "owned_by": "lmmock"}]}

    @app.get("/__lmmock/api/info")
    async def info() -> dict[str, Any]:
        return {"name": "LMMock", "version": __version__, "providers": ["openai", "anthropic"], "operations": ["chat", "responses", "messages"], "data_dir": str(store.path.parent), "rules": len(store.list_rules())}
    ...
    async def preview(request: Request) -> Response:
        ...
        mapping = {"openai_chat": ("openai", "chat"), "openai_responses": ("openai", "responses"), "anthropic_messages": ("anthropic", "messages")}

...
function buildPlaygroundRequest() {
  ...
  if (sessionStorage.getItem('lmmock-api-key')) headers.authorization = 'Bearer ••••••••';
...
$('access-key-form').addEventListener('submit', async (event) => {
  ...
  $('access-key').value = ''; $('access-key-dialog').close(); renderPlaygroundRequest(); toast(t('apiKeySaved')); await initialize();
});

...
The host can also be set with `LMMOCK_HOST`. The optional Mock API key is configured and displayed directly in the web UI, and stored as plain text in the local SQLite database. When set, `/v1/*` model endpoints require it as either `Authorization: Bearer <key>` or `x-api-key: <key>`.
